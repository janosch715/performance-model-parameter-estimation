...
public class UtilizationEstimation {
	...
	private static class ServiceCallRdEstimation {
		...
		public ServiceCallRdEstimation(
				ServiceExecutionItem serviceCall,
	...
	private final Set<String> ignoredResourceDemandIds;
	...
	private final List<ServiceCallRdEstimation> estimations;
	
	public UtilizationEstimation(
			Set<String> ignoredResourceDemandIds, 
			Repository pcmRepository, 
			List<ServiceExecutionItem> serviceCalls,
		...
		this.ignoredResourceDemandIds = ignoredResourceDemandIds;
		...
		this.estimations = new ArrayList<UtilizationEstimation.ServiceCallRdEstimation>();
		
		this.serviceIdToSeff = 
			this.pcmRepository.getComponents__Repository().stream()
				...
				.map(component -> (ResourceDemandingSEFF)component)
	...
	private void estimate() {
		for (ServiceExecutionItem serviceCall : this.serviceCalls) {
			Map<ProcessingResourceType, Double> estimatedRds = 
				this.estimateResourceDemand(serviceCall.getRecord().getServiceId(), serviceCall.getParameters());
			ServiceCallRdEstimation item =
				new ServiceCallRdEstimation(serviceCall, estimatedRds);
			this.estimations.add(item);
		}
	}
	
	private Map<ProcessingResourceType, Double> estimateResourceDemand(
			String serviceId, 
	...
	private void estimateSeffResourceDemand(
			ResourceDemandingBehaviour seff, 
			ServiceParameters serviceParameters,
			Map<ProcessingResourceType, Double> resourceDemands) {
	...
	private void estimateResourceDemand(
		...
		while (true) {
			if (currentAction instanceof BranchAction) {
				currentAction = this.estimateBranchResourceDemand((BranchAction)currentAction, serviceParameters, resourceDemands);
			} else if (currentAction instanceof LoopAction) {
				currentAction = this.estimateLoopResourceDemand((LoopAction)currentAction, serviceParameters, resourceDemands);
			} else if (currentAction instanceof InternalAction) {
				currentAction = this.estimateInternalActionResourceDemand((InternalAction)currentAction, serviceParameters, resourceDemands);
			} else if (currentAction instanceof StopAction) {
				return;
			} else {
				throw new UnsupportedOperationException("Dont know how to handle " + currentAction.eClass().getName() + ".");
			}
		}
	}
	
	private AbstractAction estimateInternalActionResourceDemand(
			InternalAction internalAction, 
			ServiceParameters serviceParameters,
			Map<ProcessingResourceType, Double> resourceDemands) {
		for (ParametricResourceDemand rd : internalAction.getResourceDemand_Action()) {
			if (this.ignoredResourceDemandIds.contains(internalAction.getId())) {
				continue;
			}
			double estimatedRd = 
					this.rdEstimation.estimateResourceDemand(internalAction.getId(), rd, serviceParameters);
	...
	private AbstractAction estimateBranchResourceDemand(
			BranchAction branchAction, 
			ServiceParameters serviceParameters,
			Map<ProcessingResourceType, Double> resourceDemands) {
		AbstractBranchTransition estimatedBranch = 
				this.branchEstimation.estimateBranch(branchAction, serviceParameters);
	...
	private AbstractAction estimateLoopResourceDemand(
			LoopAction loopAction, 
			ServiceParameters serviceParameters,
			Map<ProcessingResourceType, Double> resourceDemands) {
		double iterations = this.loopEstimation.estimateIterations(loopAction, serviceParameters);
		Map<ProcessingResourceType, Double> innerLoopResourceDemands = 
				new HashMap<ProcessingResourceType, Double>();
	...
	private static void addResourceDemands(
			Map<ProcessingResourceType, Double> rds, 
	...
	private static void multiplyResourceDemands(
			Map<ProcessingResourceType, Double> rds, 
			double factor) {
	...
	private static StartAction findStartAction(ResourceDemandingBehaviour seff) {
		List<StartAction> foundStartActions = 
			seff.getSteps_Behaviour().stream()
				.filter(StartAction.class::isInstance)
				.map(startAction -> (StartAction)startAction)
				.collect(Collectors.toList());
		
		if (foundStartActions.size() == 0) {
			throw new IllegalArgumentException(
					"The seff " + seff.getId() + " does not have a start action.");
		}
		
		if (foundStartActions.size() > 1) {
			throw new IllegalArgumentException(
					"The seff " + seff.getId() + " has multiple start actions.");
		}
